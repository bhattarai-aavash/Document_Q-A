...
def inference_main():
    ...
    if uploaded_file is not None:
        ...
        if file_extension == "pdf":
            ...
            pdf_images = pdf_to_images(pdf_bytes, "files")
            num_pages = len(pdf_images)

            if num_pages == 1:
                img_path = os.path.join("files", "output_image_0.png")
                st.image(img_path, caption="Page 1", use_column_width=True)
            else:
                # Display the current page
                current_page = st.sidebar.slider("Page", 1, num_pages, 1, 1)
                img_path = os.path.join("files", f"output_image_{current_page - 1}.png")
                st.image(img_path, caption=f"Page {current_page}", use_column_width=True)
